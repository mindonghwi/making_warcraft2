...
void GNOMISH_INVENTOR::create(int nLeft, int ntop, int nWidth, int nHeight, int nHp, float fBuildingTimer, int nFrameCount, const string & strImgKey)
{
	BUILD::create(nLeft, ntop, nWidth, nHeight, nHp, fBuildingTimer, nFrameCount, strImgKey);
	_nUnitMask += BUILDMGR::E_UNITMASK::E_BOMBER;
	_nUnitMask += BUILDMGR::E_UNITMASK::E_RECONNAISSANCE;
...
void GNOMISH_INVENTOR::release()
{
	setImage(nullptr);

}

void GNOMISH_INVENTOR::render(HDC hdc)
{
